...
namespace game
{
    ...
    /// write players name to a stream
    ...
    /// Tell server about leaving or entering the editmode.
    ...
    /// get client player model ID from client id
    ...
    /// get ui icon of player depending on player model and spectator mode
    ...
    /// @param arg is either already the cn, or the clients name (case doesn't matter)
    ...
    void setteam(const char *name_or_cn, const char *team)
    {
        int i = parseplayer(name_or_cn);
        if(i>=0) addmsg(N_SETTEAM, "ris", i, team);
    }
    COMMAND(setteam, "ss");
    ...
    /// cn and sessionid are used to hash: servers do store the hash value, too.
    /// @see hashpassword
    ...
    /// request to change server master mode.
    ICOMMAND(mastermode, "i", (int *val), addmsg(N_MASTERMODE, "ri", *val));

    /// toggle own spectator or put player into spectator
    ...
    /// ask the server to acquire CRC32 checksum of all player's maps
    ...
    /// Initial package containing the playername, playermodel and a session password
    void sendintro()
    {
